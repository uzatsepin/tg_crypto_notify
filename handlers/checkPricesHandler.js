...
addCoinHandler.callbackQuery(/price_/, async ctx => {
	...
	try {
		...
		await ctx.editMessageText(
			`Данные по валюте <b>${coinData.name}</b>: \n\n🌏 Мировой рейтинг – <b>${
				coinData.rank
			}</b>\n\n🤑 Цена в ${userCurrency} – <b>${coinData.price.toFixed(4)}</b>\n📈 Изменения цены за <i>час</i> – <b>${
				coinData.priceChange1h
			}${getSymbolForCurrency(userCurrency)}</b>\n📈 Изменения цены за <i>день</i> – <b>${coinData.priceChange1d}${getSymbolForCurrency(userCurrency)}</b>\n📈 Изменения цены за <i>неделю</i> – <b>${
